...
# This block of code obtains V'_delta(x), based on the explicit formula in the paper
def V_delta_prime(x, delta):
    # Defining some terms used in the explicit formula
    term1 = fdx(x - delta, u_0(x - delta), delta)
    term2 = fdy(x - delta, u_0(x - delta), delta) * u_0Prime(x - delta)
    term3 = u_0(x - 2 * delta) * k(-delta, delta)

    # Defining the first integral term (from x - 2*delta to x)
    def integrand1(y):
        return u_0Prime(x - delta) * k(y - x + delta, delta)

    integral1, _ = quad(integrand1, x - 2 * delta, x)

    # Defining the second integral term (from x - 2*delta to 0)
    def integrand2(y):
        return u_0(y) * kPrime(y - x + delta, delta)
...
# This is my method for checking uniform convergence between iterations
# to know if we can terminate earlier.
